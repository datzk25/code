...
def start_tag(user_id, idbox, cookie, delay):
    if not os.path.exists("tag.txt"):
        return "Không tìm thấy file tag.txt."
    try:
        messenger = Messenger(cookie)
    except Exception as e:
        return f"Lỗi cookie: {str(e)}"
    ...
    if not messages:
        return "File tag.txt không có nội dung."

    def loop_tag():
        index = 0
        while (user_id, idbox) in tag_threads:
            message = messages[index % len(messages)]
            # Thêm tag vào tin nhắn
            tag_message = f"@{user_id} {message}"  # Giả sử tag là tên người dùng (user_id)
            success = messenger.gui_tn(idbox, tag_message)
            print(f"Gửi tin nhắn {'Thành công' if success else 'Thất bại'}")
            time.sleep(delay)
            index += 1

    key = (user_id, idbox)
    thread = threading.Thread(target=loop_tag)
    tag_threads[key] = thread
    tag_start_times[key] = time.time()
    thread.start()
    return "Đã bắt đầu tag."
...
@bot.command()
async def tag(ctx, idbox: str, cookie: str, delay: int):
    if ctx.author.id not in allowed_users and ctx.author.id != ADMIN_ID:
        return await ctx.send("Bạn chưa thuê bot ?")
    result = start_tag(ctx.author.id, idbox, cookie, delay)
    await ctx.send(result)    

@bot.command()
async def stoptag(ctx, idbox: str):
    key = (ctx.author.id, idbox)
    if key in tag_threads:
        tag_threads.pop(key)
        tag_start_times.pop(key)
        await ctx.send(f"Đã dừng nhây vào {idbox}.")
    else:
        await ctx.send("Không có tab nhây nào đang chạy.")

@bot.command()
async def tabtag(ctx):
    msg = "**Danh Sách Tab nhây:**\n\n"
    count = 0
    for (uid, ib), start in tag_start_times.items():
        if uid == ctx.author.id:
            uptime = format_duration(time.time() - start)
            msg += f"**{ib}:** {uptime}\n"
            count += 1
