...
def start_spam(user_id, idbox, cookie, message, delay):
    ...
    def loop_treo():
        ...
        while (user_id, idbox) in treo_threads:
            result = messenger.gui_tn(idbox, message)
            if result.get("success"):
                print(f"[{idbox}] Gửi thành công")
                fail_count = 0
            else:
                print(f"[{idbox}] Thất bại: {result.get('error')} - {result.get('error_description')}")
                fail_count += 1
                if "cookie" in result.get("error_description", "").lower() or "403" in result.get("error_description", ""):
                    print(f"[{idbox}] Lỗi nghiêm trọng, dừng thread.")
                    treo_threads.pop((user_id, idbox), None)
                    treo_start_times.pop((user_id, idbox), None)
                    break
                if fail_count >= max_fail:
                    print(f"[{idbox}] Lỗi quá nhiều lần liên tiếp, dừng.")
                    treo_threads.pop((user_id, idbox), None)
                    treo_start_times.pop((user_id, idbox), None)
                    break
            time.sleep(delay)
    ...
    def loop_nhay():
        ...
        while (user_id, idbox) in nhay_threads:
            ...
            if result.get("success"):
                ...
                if "cookie" in result.get("error_description", "").lower() or "403" in result.get("error_description", ""):
                    print(f"[{idbox}] Lỗi nghiêm trọng, dừng thread.")
                    nhay_threads.pop((user_id, idbox), None)
                    nhay_start_times.pop((user_id, idbox), None)
                    break
                if fail_count >= max_fail:
                    print(f"[{idbox}] Lỗi quá nhiều lần liên tiếp, dừng.")
                    nhay_threads.pop((user_id, idbox), None)
...
def start_reo(user_id, name, cookie, delay):
    ...
    def loop_reo():
        fail_count = 0
        max_fail = 10
        while (user_id, idbox) in reo_threads:
            result = messenger.gui_tn(idbox, message)
            if result.get("success"):
                print(f"[{idbox}] Gửi thành công")
                fail_count = 0
                ...
                if "cookie" in result.get("error_description", "").lower() or "403" in result.get("error_description", ""):
                    print(f"[{idbox}] Lỗi nghiêm trọng, dừng thread.")
                    reo_threads.pop((user_id, idbox), None)
                    reo_start_times.pop((user_id, idbox), None)
                    break
                if fail_count >= max_fail:
                    print(f"[{idbox}] Lỗi quá nhiều lần liên tiếp, dừng.")
                    reo_threads.pop((user_id, idbox), None)
    ...
    def loop_chui():
        ...
        while (user_id, idbox) in chui_threads:
            ...
            if result.get("success"):
                ...
                if "cookie" in result.get("error_description", "").lower() or "403" in result.get("error_description", ""):
                    print(f"[{idbox}] Lỗi nghiêm trọng, dừng thread.")
                    chui_threads.pop((user_id, idbox), None)
                    chui_start_times.pop((user_id, idbox), None)
                    break
                if fail_count >= max_fail:
                    print(f"[{idbox}] Lỗi quá nhiều lần liên tiếp, dừng.")
                    chui_threads.pop((user_id, idbox), None)
    ...
    def loop_codelag():
        ...
        while (user_id, idbox) in codelag_threads:
            ...
            if result.get("success"):
                ...
                if "cookie" in result.get("error_description", "").lower() or "403" in result.get("error_description", ""):
                    print(f"[{idbox}] Lỗi nghiêm trọng, dừng thread.")
                    codelag_threads.pop((user_id, idbox), None)
                    codelag_start_times.pop((user_id, idbox), None)
                    break
                if fail_count >= max_fail:
                    print(f"[{idbox}] Lỗi quá nhiều lần liên tiếp, dừng.")
                    codelag_threads.pop((user_id, idbox), None)
